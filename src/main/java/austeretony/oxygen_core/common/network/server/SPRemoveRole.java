...
public class SPRemoveRole extends Packet {

    private int roleId;

    public SPRemoveRole() {}

    public SPRemoveRole(int roleId) {
        this.roleId = roleId;
    }
    ...
    public void read(ByteBuf buffer, INetHandler netHandler) {
        ...
        if (OxygenHelperServer.isNetworkRequestAvailable(CommonReference.getPersistentUUID(playerMP), OxygenMain.MANAGE_PRIVILEGES_REQUEST_ID)) {
            ...
            OxygenHelperServer.addRoutineTask(()->OxygenManagerServer.instance().getPrivilegesManager().removeRole(playerMP, roleId));
        }
    }
}
