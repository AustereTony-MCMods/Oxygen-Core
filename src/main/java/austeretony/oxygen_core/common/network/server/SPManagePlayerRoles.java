...
public class SPManagePlayerRoles extends Packet {

    private int ordinal, roleId;

    private UUID playerUUID;

    public SPManagePlayerRoles() {}

    public SPManagePlayerRoles(EnumAction action, int roleId, UUID playerUUID) {
    ...
    public void read(ByteBuf buffer, INetHandler netHandler) {
        ...
        if (OxygenHelperServer.isNetworkRequestAvailable(CommonReference.getPersistentUUID(playerMP), OxygenMain.MANAGE_PRIVILEGES_REQUEST_ID)) {
            ...
            if (ordinal >= 0 && ordinal < EnumAction.values().length) {
