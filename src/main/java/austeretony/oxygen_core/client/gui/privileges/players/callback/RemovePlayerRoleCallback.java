...
import austeretony.oxygen_core.client.gui.privileges.players.PlayerRolePanelEntry;
...
public class RemovePlayerRoleCallback extends AbstractGUICallback {
    ...
    public RemovePlayerRoleCallback(PrivilegesScreen screen, PlayersSection section, int width, int height) {
    ...
    public void init() {
        ...
        this.addElement(new OxygenTextLabel(4, 12, ClientReference.localize("oxygen_core.gui.privileges.callback.removePlayerRole"), EnumBaseGUISetting.TEXT_SCALE.get().asFloat(), EnumBaseGUISetting.TEXT_ENABLED_COLOR.get().asInt()));
    ...
    private void loadPlayerRoles() {
        ...
        PlayerSharedData sharedData = OxygenManagerClient.instance().getPrivilegesManager().getPlayerData(this.section.getCurrentPlayerUUID());
        int roleId;
        for (int i = 0; i < OxygenMain.MAX_ROLES_PER_PLAYER; i++) {
            ...
            if (roleId != OxygenMain.DEFAULT_ROLE_INDEX)
                this.rolesPanel.addEntry(new PlayerRolePanelEntry(OxygenManagerClient.instance().getPrivilegesManager().getRole(roleId), this.section.getCurrentPlayerUUID()));
        }
    }
