package austeretony.oxygen_core.client.gui.privileges.management.privileges.callback;
...
public class RemoveDefaultPrivilegeCallback extends AbstractGUICallback {

    private final PrivilegesManagementScreen screen;

    private final DefaultPrivilegesSection section;

    private OxygenKeyButton confirmButton, cancelButton;

    public RemoveDefaultPrivilegeCallback(PrivilegesManagementScreen screen, DefaultPrivilegesSection section, int width, int height) {
    ...
    public void init() {
        ...
        this.addElement(new OxygenTextLabel(4, 12, ClientReference.localize("oxygen_core.gui.privileges.management.callback.removeDefaultPrivilege"), EnumBaseGUISetting.TEXT_SCALE.get().asFloat(), EnumBaseGUISetting.TEXT_ENABLED_COLOR.get().asInt()));
    ...
    private void confirm() {
        OxygenManagerClient.instance().getPrivilegesManager().removeDefaultPrivilegeSynced(this.section.getCurrentPrivilegeEntry().getWrapped());
        this.close();
    }
