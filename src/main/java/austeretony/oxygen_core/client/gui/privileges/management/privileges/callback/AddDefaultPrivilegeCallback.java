...
public class AddDefaultPrivilegeCallback extends AbstractGUICallback {
    ...
    public AddDefaultPrivilegeCallback(PrivilegesManagementScreen screen, DefaultPrivilegesSection section, int width, int height) {
    ...
    public void init() {
        ...
        this.addElement(new OxygenTextLabel(4, 12, ClientReference.localize("oxygen_core.gui.privileges.management.callback.addDefaultPrivilege"), EnumBaseGUISetting.TEXT_SCALE.get().asFloat(), EnumBaseGUISetting.TEXT_ENABLED_COLOR.get().asInt()));
    ...
    private void confirm() {
        if (!this.valueField.isDragged()) {
            OxygenManagerClient.instance().getPrivilegesManager().addDefaultPrivilegeSynced(
