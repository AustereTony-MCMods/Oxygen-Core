...
public class RemovePlayerCallback extends AbstractGUICallback {
    ...
    public RemovePlayerCallback(PrivilegesManagementScreen screen, RolesSection section, int width, int height) {
    ...
    public void init() {
        ...
        this.addElement(new OxygenTextLabel(6, 23, ClientReference.localize("oxygen_core.gui.privileges.management.callback.removePlayerRole.request"), EnumBaseGUISetting.TEXT_SUB_SCALE.get().asFloat(), EnumBaseGUISetting.TEXT_ENABLED_COLOR.get().asInt()));
    ...
    private void confirm() {
        OxygenManagerClient.instance().getPrivilegesManager().removeRoleFromPlayerSynced(this.section.getCurrentPlayerEntry().getWrapped(), this.section.getCurrentRoleEntry().getWrapped().getId());
        this.close();
    }
